...
func _physics_process(delta: float) -> void:
	...
	# Handle jump.
	if Input.is_action_just_pressed("NextDialouge") and is_on_floor():
		velocity.y = JUMP_VELOCITY
		SPEED = SPEED + 5
		%jumpTimer.start(0.2)
	if Input.is_action_just_pressed("NextDialouge") and not is_on_floor():
...
func _on_winbox_body_entered(body: Node2D) -> void:
	get_tree().change_scene_to_file("res://end_comic.tscn")
